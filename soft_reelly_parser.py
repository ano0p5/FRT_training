    ...
    def parse_project_data(self, data):
        ...
        parsed_data = {
            ...
            "payment_plan": ", ".join(
                ...
                for sublist in (data.get("Payment_plans") or []) if isinstance(sublist, list)
            ...
            "facilities": ", ".join(
                ...
                for sublist in (data.get("Facilities") or []) if isinstance(sublist, list)
                for facility in sublist if isinstance(facility, dict) and facility.get("Name")
            ),

            "map_points": ", ".join(
                f"{str(point['Point_name'])}-{str(point['Distance_km'])}"
            ...
            "architecture_images": ", ".join([str(img.get("url", "")) for img in data.get("Architecture", []) if isinstance(img, dict) and img.get("url")]),
            "interior_images": ", ".join([str(img.get("url", "")) for img in data.get("Interior", []) if isinstance(img, dict) and img.get("url")]),
            "lobby_images": ", ".join([str(img.get("url", "")) for img in data.get("Lobby", []) if isinstance(img, dict) and img.get("url")]),
            "layout_images": ", ".join([str(img.get("url", "")) for img in data.get("Layouts_preview_img", []) if isinstance(img, dict) and img.get("url")]),
            ...
            "developer_logo": str(next(
                (dev.get("Logo_image", [{}])[0].get("url") for dev in data.get("Developer", []) if isinstance(dev, dict) and "Logo_image" in dev and isinstance(dev["Logo_image"], list)),
                ""
            )),
...
def main():
    ...
    project_fetcher = ProjectFetcher(
        ...
        project_data_collection="soft_reelly_final",  
        headers=headers,
        payload=payload
    )
